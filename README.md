...
Vercel Cron 대신 **EC2 서버의 crontab**이 매일 정해진 시간에 `/api/sync`를 호출합니다 (`vercel.json`은 더 이상 쓰지 않아 삭제했습니다). EC2에 SSH로 접속해 아래처럼 등록합니다.
...
30 9 * * 1-5 curl -s -H "Authorization: Bearer <CRON_SECRET값>" http://localhost:3000/api/sync >> /home/ubuntu/sync.log 2>&1
```
