...
5. `.env.example`을 기준으로 로컬 `.env`와 Vercel 환경변수를 채운다.
6. DB 스키마를 적용한다.
7. Vercel에 배포한다.
...
아래 값들은 `.env.example`과 Vercel 환경변수에 같은 이름으로 입력합니다. 로컬 `.env`는 개발용 값, Vercel 환경변수는 배포용 값을 넣습니다.
...
배포 환경에서는 `/api/sync`를 아무나 호출하지 못하도록 `CRON_SECRET`을 반드시 설정합니다.
...
생성된 값을 로컬 `.env`와 Vercel 환경변수에 같은 값으로 넣습니다.

```dotenv
CRON_SECRET=생성한_긴_랜덤_문자열
```

`CRON_SECRET`이 설정되면 `/api/sync` 호출에는 아래 헤더가 필요합니다.

```http
Authorization: Bearer <CRON_SECRET>
```

로컬 개발 중 `CRON_SECRET`을 비워두면 인증 없이 `/api/sync`를 호출할 수 있습니다. 배포 환경에서는 비워두지 않습니다.

## Vercel 배포 절차

1. GitHub에 현재 저장소를 올립니다.
2. Vercel에서 새 프로젝트를 만들고 GitHub 저장소를 연결합니다.
3. Framework Preset은 Next.js로 둡니다.
4. Build Command는 `npm run build`를 사용합니다.
5. Vercel Project Settings에서 Production 환경변수를 입력합니다.
6. 외부 Postgres에 스키마를 적용합니다.
7. Deploy를 실행합니다.
8. 배포 URL을 `APP_BASE_URL`에 반영한 뒤 다시 배포합니다.

Vercel 환경변수에는 최소한 아래 값을 넣습니다.

```dotenv
DATABASE_URL=운영_Postgres_접속문자열
PGSSLMODE=require
G2B_SERVICE_KEY=공공데이터포털_서비스키
G2B_STANDARD_API_BASE_URL=http://apis.data.go.kr/1230000/ao/PubDataOpnStdService
G2B_BID_PUBLIC_API_BASE_URL=http://apis.data.go.kr/1230000/ad/BidPublicInfoService
APP_BASE_URL=https://배포도메인
SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
CRON_SECRET=생성한_긴_랜덤_문자열
AI_PROVIDER=mock
OPENAI_API_KEY=
...
Vercel Cron 설정은 이미 `vercel.json`에 들어 있습니다.
...
이 설정은 UTC 기준 평일 00:30, 한국시간 기준 평일 오전 9:30에 `/api/sync`를 호출합니다.
...
`CRON_SECRET`이 설정된 배포 환경에서는 아래처럼 호출합니다.

```powershell
curl.exe -H "Authorization: Bearer <CRON_SECRET>" https://배포도메인/api/sync
```
...
인증 없이 호출했을 때 `401 Unauthorized`가 나오면 `CRON_SECRET` 보호가 정상입니다.
...
`DATABASE_URL`이 비어 있거나 Vercel 환경변수에 입력되지 않은 상태입니다. 로컬은 `.env`, 배포는 Vercel Project Settings의 Environment Variables를 확인합니다.
...
`SLACK_WEBHOOK_URL`이 비어 있으면 알림은 의도적으로 건너뜁니다. 값이 있는데 실패하면 Slack Webhook URL, 채널 권한, Vercel Runtime Logs를 확인합니다.

### `/api/sync`가 401을 반환함

`CRON_SECRET`이 설정된 상태입니다. 수동 호출 시 `Authorization: Bearer <CRON_SECRET>` 헤더를 붙입니다.
